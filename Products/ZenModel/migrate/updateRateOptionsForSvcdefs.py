##############################################################################
#
# Copyright (C) Zenoss, Inc. 2017, all rights reserved.
#
# This content is made available according to terms specified in
# License.zenoss under the directory where your Zenoss product is installed.
#
##############################################################################

import logging
log = logging.getLogger("zen.migrate")

import Migrate
import servicemigration as sm
sm.require("1.1.10")

from Products.ZenModel.ZMigrateVersion import SCHEMA_MAJOR, SCHEMA_MINOR, SCHEMA_REVISION


class UpdateRateOptionsForSvcdefs(Migrate.Step):
    """
    Update/Add rateOptions for counters.
    Remove reset values.
    """

    version = Migrate.Version(SCHEMA_MAJOR, SCHEMA_MINOR, SCHEMA_REVISION)

    def cutover(self, dmd):

        try:
            ctx = sm.ServiceContext()
        except sm.ServiceMigrationError:
            log.info("Couldn't generate service context, skipping.")
            return

        changed = False

        for svc in ctx.services:
            mprofile = svc.monitoringProfile
            for gconfig in mprofile.graphConfigs:
                for dp in gconfig.datapoints:
                    if dp.rate == True or (dp.rate == False and dp.rateOptions):
                        changed = True
                        dp.rateOptions = {
                            "counter": True
                        }
                        log.info("[{0}] {1}.rateOptions set to {2}".format(svc.name, dp.name, dp.rateOptions))
            for mconfig in mprofile.metricConfigs:
                metrics = mconfig.metrics
                for m in metrics:
                    if m.counter == True and m.resetValue != 0:
                        changed = True
                        m.resetValue = 0
                        log.info("[{0}] {1}.resetValue set to {2}".format(svc.name, m.name, m.resetValue))

        if changed:
            ctx.commit()

UpdateRateOptionsForSvcdefs()
