...
from Products.ZenUtils.mib.mibfile import MibFile
from Products.ZenUtils.mib.config import SMIConfigFile
from Products.ZenUtils.mib.tools import SMIDumpTool
...
class ZenMibOld(ZCmdBase):
    ...
    def populateDependencyMap(self, importFileNames, depFileNames):
        ...
        @param importFileNames: fully qualified file names of MIB
            files to import
        ...
        for fileName in depFileNames.union(importFileNames):
            ...
            if not mibDependencies:
                self.log.warn(
                    "Unable to parse information from %s -- skipping",
                    fileName
                )
                continue
    ...
    def savePythonCode(self, pythonCode, fileName):
        ...
        if not os.path.exists(self.options.pythoncodedir):
            ...
            self.log.warn(
                'The directory %s to store converted MIB file code '
                'does not exist.', self.options.pythoncodedir
            )
            return
        try:
            pythonFileName = os.path.join(
                self.options.pythoncodedir,
            ...
            self.log.warn(
                'Could not output converted MIB to %s', pythonFileName
            )

    def generatePythonFromMib(
            self, fileName, dependencyFileNames, mibNamesInFile):
        """
        ...
        # then proc.wait() blocks waiting for the OS pipe buffer to accept
        # more data (ie forever).  Poll from time to time to avoid the
        # problem with large MIBs.
        pythonCode = ''
        warnings = ''
        ...
        if warnings:
            self.log.debug(
                "Found warnings while trying to import MIB:\n%s", warnings
            )
    ...
    def evalPythonToMibs(self, pythonCode, name):
        ...
        def executePythonCode(pythonCode, name):
            ...
            try:
                ...
                self.log.exception(
                    "Unable to import Pythonized-MIB: %s", name
                )
        ...
        if "TRAP-TYPE" in pythonCode and not self.options.removemiddlezeros:
            self.log.warn(
                "This MIB file contains a type of OID (TRAP-TYPE) that, "
                "when parsed, inserts an extra zero into the OID causing "
                "SNMP trap lookups to fail. Please run zenmib with the "
                "'--removemiddlezeros' flag option to avoid this problem."
            )
        ...
        for name, values in pythonMib[leafType].items():
            ...
            if self.options.removemiddlezeros:
                ...
                if '.0.' in oid:
                    ...
                    self.log.warn(
                        "Found a zero index in OID '%s' "
                        "-- converting to '%s'", oid, values['oid']
                    )
            ...
            try:
                ...
                try:
                    self.log.warn(
                        "Unable to add %s id '%s' as this name is reserved "
                        "for use by Zope", leafType, name
                    )
                    ...
                    self.log.warn(
                        ...
                        name, newName, leafType
                    )
                except Exception:
                    self.log.warn(
                        "Unable to add %s id '%s' -- skipping", leafType, name
                    )
            else:
    ...
    def loadMibFile(self, mibFileObj, dmdMibDict):
        ...
        # fileName.
        pythonMibs = self.generatePythonFromMib(
            fileName, dependencyFileNames, mibNamesInFile
        )
        if not pythonMibs:
            return False
    ...
    def getMibsToImport(self):
        ...
        if loadFileNames:
            self.log.debug(
                "Will attempt to load the following files: %s", loadFileNames
            )
        else:
    ...
    def main(self):
        ...
        if not os.path.exists(self.options.mibsdir):
            self.log.error(
                "'mibsdir' path not found: %s", self.options.mibsdir
            )
            sys.exit(1)
        ...
        mibFileObjs = self.populateDependencyMap(
            requestedFiles, mibDepFileNames
        )
        ...
        for mibFileObj in mibFileObjs:
            try:
                ...
                self.log.exception(
                    "Failed to load MIB: %s", mibFileObj.fileName
                )
...
class ZenMib(ZCmdBase):
        ...
        for mibfile in mibfiles:
            try:
                ...
                with SMIConfigFile(path=paths) as cfg:
                    ...
                    # returns list of dicts with MIB data
                    mibs = tool.run(mibfile)
                    for mib in mibs:
                        self._loadMIB(mib)
            except Exception as ex:
                self._logException(
                    "Failed to load MIB file %s: %s", mibfile, ex
                )
    ...
    def _loadMIB(self, mib):
        """
        """

    def _loadMibFile(self, mibfile):
        """
        """
        # 1. generate the config file
        # 2. run smidump to generate the Python code
        # 3. 'Parse' the Python code for the MIB dictionaries
        # 4. Load each dictionary into DMD
        paths = [zenPath("share", "mibs"), self.options.mibdepsdir]
        with SMIConfigFile(path=paths) as cfg:
            tool = SMIDumpTool(config=cfg)
            mibs = tool.run(mibfile)  # returns list of dicts with MIB data
            for mib in mibs:
                self._loadMIB(mib)
    ...
    def parseOptions(self):
        ...
        # Verify MIB directory is valid. Fail if not.
        if not os.path.exists(self.options.mibsdir):
            self.log.error(
                "'mibsdir' path not found: %s", self.options.mibsdir
            )
        ...
        # Verify that output directory for generated Python code exists.
        if not os.path.exists(self.options.pythoncodedir):
            self.log.error(
                "'pythoncodedir' path not found: %s",
                self.options.pythoncodedir
            )

        # Verify that the download directory exists.
        if not os.path.exists(self.options.downloaddir):
            self.log.error(
                "'downloaddir' path not found: %s", self.options.downloaddir
            )

        # Verify that the extraction directory exists.
        if not os.path.exists(self.options.extractdir):
            self.log.error(
                "'extractdir' path not found: %s", self.options.extractdir
            )

        # Verify that arguments are valid files.
        nonFiles = filter(lambda f: not os.path.isfile(f), self.args)
        if nonFiles:
            self.log.error("File(s) not found: %s", ', '.join(nonFiles))

        # If no arguments are given, load the files found in mibsdir
        ...
        if self.args:
            ...
            loadFileNames = list(_pathwalker(self.options.mibsdir))
